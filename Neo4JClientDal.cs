namespace GraphDbExamples
{
    ...
    public class Neo4JClientDal
    {
        ...
        public void CreatePerson_ACTEDIN_Movie(Person person, ActedIn actedIn, Movie movie) // актори фільмів
        {
            this.graphClient.Cypher.Merge("(p:Person:Actor { name: {name}, born:{born} })")
                ...
                .ExecuteWithoutResults();  // виконати параметри
        }

        public void CreatePerson_DIRECTED_Movie(Person person, Movie movie) //Деректори
        {
            this.graphClient.Cypher.Merge("(p:Person { name: {name}, born:{born} })")
                .OnCreate()
                .Set("p = {person}")  // вставити
                .Set("p :Director")
                .WithParams(new { person.name, person.born, person })
                .Merge("(m:Movie { title: {title}, released:{released} })") //об'єднати
                .OnCreate()
                .Set("m = {movie}")
                .WithParams(new { movie.title, movie.released, movie })
                .Merge("(p)-[:DIRECTED]->(m)")
        ...
        public void CreatePerson_PRODUCED_Movie(Person person, Movie movie) // Продюсери
        {
            this.graphClient.Cypher.Merge("(p:Person { name: {name}, born:{born} })")
                .OnCreate()
                .Set("p = {person}")
        ...
        public void CreatePerson_WROTE_Movie(Person person, Movie movie) //Хто писав кіно
        {
            this.graphClient.Cypher.Merge("(p:Person { name: {name}, born:{born} })")
                .OnCreate()
                .Set("p = {person}")
        ...
        public void DeleteACTED_INRelationship(string personName, string movieTitle) // звязки
        {
        ...
        public void ExecuteScopedTransactions(List<ICypherFluentQuery> transactionList) //Виконати скасовані транзакції
        {
        ...
        public List<string> GetMoviesByActor(string movie)
        {
            List<string> ActorsMovieTitles =
                this.graphClient.Cypher.Match("(movie:Movie {title: {titleParam}})")
                    .WithParam("nameParam", movie)
                    .Return(m => m.As<Movie>().title)
                    .OrderBy("m.title")
                    .Results.ToList();
            return ActorsMovieTitles;
        ...
        public List<string> GetActorsRolesInMovie(string actorsName, string movieTitle)
        {
            ...
            catch (InvalidOperationException) //thrown коли нічого не найдено
            {
        ...
        public async Task<IEnumerable<Crew>> GetCrewOfMovieAsync(string movieTitle)
        {
            IEnumerable<Crew> movieCrew =
                await
                    this.graphClient.Cypher.Match("(movie:Movie {title: {titleParam}})")
                        .OptionalMatch("(person:Person)-[r:ACTED_IN]->(movie:Movie)")
                        .WithParam("titleParam", movieTitle)
                        .Return((person, r) => new Crew { Name = person.As<Person>().name, Role = r.Type() })
                        .OrderBy("person.name")
                        .ResultsAsync;
